...
There are many examples where paying for proprietary software makes sense because there is no close FOSS substitute. I am a huge fan of Obsidian, the markdown-based note-taking app. One of the reasons I think it's great is that you never lose control of your data or the format it's in: all of your notes are just markdown files and the Obsidian app just gives a fancy way to edit and view that content. The Obsidian app happens to be free but it's not open source. I would similarly say there's no FOSS equivalent of the incredible integration of hardware and software that Apple has achieved with the Macbook. But, equally, there are plenty of areas where direct FOSS substitutes exist.

Efficiency due to FOSS comes through a bunch of channels, in my view:

- The 'F' stands for free! Productivity is output per unit of input, so free is pretty good.[^7]
- Widely used open source software is less likely to have critical security flaws. If it did have, they would have already been exploited and fixed.
- Again because it's open source, you actually know what it's doing!
- Very much related to the above, and somewhat counter-intuitively, it's more private. You can see if there's a backdoor. A proprietary software vendor knows, at the very least, that you bought the software: a maintainer of a FOSS package may not even know you exist.
- By dint of being free and widely used, there are a lot of people who use specific open source software, which means there are more ideas for how to improve it, and guides on how to use it. It's a positive network effect, basically.
...
- Missing a feature you'd like for your org in a specific library? If it's reasonable and within-scope, you can probably get it added by the maintainers. Or your org can simply add it for themselves.
- There's also a diversity, equality, and inclusion benefit. Lots of students, especially those from poorer backgrounds, can't afford expensive proprietary software but can use FOSS instead (as a student, I used an earlier iteration of [LibreOffice](https://www.libreoffice.org/)). Those with more resources may have been able to use proprietary software before starting work, getting an edge; FOSS helps level the field. Similarly, if you're trying to be a leading public sector institution and inspire other institutions around the world, you'll go further if you use FOSS because it's accessible in every country.
...
::: {.callout-note}
This section is significantly more technical than the others and some people may want to skip it.
:::
