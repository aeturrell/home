...
- Widely-used open source software is less likely to have critical security flaws. If it did have security issues, they *probably* would have already been exploited and then fixed *because* it's open---I call this the weak form of the open source security hypothesis.
...
- There's also a diversity, equality, and inclusion benefit. Lots of students, especially those from poorer backgrounds, can't afford expensive proprietary software but can use FOSS instead (as a student, I used a version of [LibreOffice](https://www.libreoffice.org/)). Those with more resources may have been able to use proprietary software before starting work, which gives them an edge when they start work; FOSS helps level the field. Similarly, if you're trying to be a leading public sector institution and inspire other institutions around the world, you'll go further if you use FOSS because it's accessible in every country.
...
There are well-developed packages to help create and understand synthetic data:
...
::: {.callout-note}
This section is significantly more technical than the others and some readers may want to skip it.
:::
...
A lot of public sector organisations are using Microsoft and its suite of office tools. These are, happily, scriptable with code. In particular, Microsoft has tools that aid with automating the use of its office software, including:
...
One more example: people are always emailing documents to read. As noted in the previous post though, email is a bad way to communicate source information. Instead, you can have a code setup to watch a Sharepoint folder for any new documents dropped into it. When a new document arrives, the people who need to read the document get a notification.

If you're on Google Docs, there's an [API for that too](https://developers.google.com/docs/api/reference/rest).

And there are plenty of generic office-adjacent code tools that could be used to improve efficiency:
...
Power Automate is expensive, but the benefit is that you can create automated flows using point and click, and just filling in a few details. If you're doing something fairly standard, ie something that is already in one of the templated examples, it's extremely convenient. Some examples of what can be automated include:
